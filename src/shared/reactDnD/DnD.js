...
class DnD extends Component {
	...
	addTask(event) {
		...
		const newList = Object.assign(this.state.tasks, {
			[newTask.id]: newTask,
		});
	...
	addAction(event) {
		...
		const newList = Object.assign(this.state.actions, {
	...
	deleteAction(e) {
		...
		const taskId = document.getElementById(actionId).parentNode.parentNode
			.parentNode.id;

		const newActionIds = Array.from(this.state.tasks[taskId].actionIds);
		const newActions = this.state.actions;
		const actionIndex = newActionIds.findIndex(
					...
					{(provided) => (
						...
						>
							...
							{this.state.taskOrder.map((taskId, index) => {
								...
								const actions = task.actionIds.map(
